...
def compute_hml_factor(input_ccm_filename: str,
    ...
    logging.info("Creating the positive book equity to December market equity portfolio boolean...")
    crsp_compustat_june['positive_book_equity_to_december_market_equity'] = np.where((crsp_compustat_june['book_equity_to_december_market_equity'] > 0) &
                                                                                     (crsp_compustat_june['market_equity'] > 0),
                                                                                     1,
                                                                                     0)

    logging.info("Creating the non-missing portfolio boolean...")
    crsp_compustat_june['non_missing_portfolio'] = np.where((crsp_compustat_june['factor_portfolio'] != ''),
                                                            1,
                                                            0)
    ...
    crsp_compustat = pd.merge(crsp,
                              crsp_compustat_june[['permanent_number', 'fama_french_year', 'size_portfolio', 'factor_portfolio', 'positive_book_equity_to_december_market_equity', 'non_missing_portfolio']],
                              how='left',
                              on=['permanent_number', 'fama_french_year'])
    
    logging.info("Keeping only the relevant data...")
    crsp_compustat = crsp_compustat[(crsp_compustat['weight'] > 0) &
                                    (crsp_compustat['positive_book_equity_to_december_market_equity'] == 1) &
                                    (crsp_compustat['non_missing_portfolio'] == 1)]
    
    logging.info("Calculating the size and value weighted returns...")
    ...
    logging.info("Calculating the HML factor...")
...
def compute_rmw_factor(input_ccm_filename: str,
    ...
    logging.info("Creating a universe of NYSE common stocks with positive book equity to December market equity and market equity...")
    ...
    logging.info("Merging the CRSP data with the CRSP Compustat June data...")
    crsp_compustat = pd.merge(crsp,
                              crsp_compustat_june[['permanent_number', 'fama_french_year', 'size_portfolio', 'factor_portfolio', 'non_missing_portfolio']],
                              how='left',
                              on=['permanent_number', 'fama_french_year'])
    ...
    # only keep the date and the xHML factor and xSHML factor
    fama_french_replicated_factors = fama_french_replicated_factors[['month_end_date', 'xRMW', 'xSRMW']]

    # rename month_end_date to date
    fama_french_replicated_factors = fama_french_replicated_factors.rename(columns={'month_end_date': 'date'})

    logging.info("Saving the Fama-French replicated factors to a CSV file...")
    fama_french_replicated_factors.to_csv(output_rmw_factor_filename,
                                          index=False)
...
def compute_umd_factor(input_crsp_filename: str,
    ...
    logging.info("Creating a universe of NYSE common stocks with positive momentum and weight...")
    ...
    logging.info("Creating the non-missing portfolio boolean...")
    crsp['non_missing_portfolio'] = np.where(crsp['factor_portfolio'] != '',
                                             1,
                                             0)
    ...
    logging.info("Calculating the UMD factor...")
    ...
    # only keep the date and the xUMD factor
...
def compare_fama_french_factors(input_fama_french_3_factors_filename: str,
    ...
    logging.info("Merging the Fama-French factors with the replicated Fama-French factors...")
    fama_french_comparision = pd.merge(fama_french_comparision,
                                        hml_factor,
                                       how='inner',
                                        on='date')
    
    logging.info("Merging the Fama-French factors with the replicated RMW factors...")
    fama_french_comparision = pd.merge(fama_french_comparision,
                                        rmw_factor,
                                       how='inner',
                                        on='date')
    
    logging.info("Merging the Fama-French factors with the replicated CMA factors...")
    fama_french_comparision = pd.merge(fama_french_comparision,
                                        cma_factor,
                                       how='inner',
                                        on='date')
    
    logging.info("Merging the Fama-French factors with the replicated UMD factors...")
    fama_french_comparision = pd.merge(fama_french_comparision,
                                        umd_factor,
                                       how='inner',
                                        on='date')
    ...
    # multiply xSMB and xHML by 100 and round to two decimal places
